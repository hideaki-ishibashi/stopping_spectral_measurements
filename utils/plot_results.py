...
def draw_pred_model(axes, al, node, time=None, y_range=None, title=None, x_label=None, y_label=None, plotCurrentData=False,isLegend=False):
    if time is None:
    ...
    if plotCurrentData:
        axes.axvline(x=dataset[0][-1, 0], ymin=-1e10, ymax=1e10, c="k", linestyle="dashed")
    axes.plot(node, pos[0], c="b", label="Mean function")
    axes.fill_between(node[:, 0], pos[0] - 2 * pos[1], pos[0] + 2 * pos[1], color="b", alpha=0.5,
...
def draw_test_error(axes, al, test_error, diff_test_error=None, time=None):
    ...
    axes.cla()
    axes.plot(range(al.init_sample_size + 1, test_error[:time].shape[0] + al.init_sample_size + 1), test_error[:time],
              c="k", label="Test error")
    if diff_test_error is not None:
        axes.plot(range(al.init_sample_size + 1, diff_test_error[:time].shape[0] + al.init_sample_size + 1),
                  diff_test_error[:time], c="b", label="Diff test errors")
    color = ["r", "g", "b"]
...
def draw_error_ratio(axes, al, time=None):
    ...
    axes.cla()
    axes.plot(
        range(al.init_sample_size + 1, al.stopping_criteria[0].error_ratio[:time].shape[0] + al.init_sample_size + 1),
        al.stopping_criteria[0].error_ratio[:time], c="k", label="Error ratio")
    axes.plot(
        range(al.init_sample_size + 1, al.stopping_criteria[0].error_ratio[:time].shape[0] + al.init_sample_size + 1),
        al.stopping_criteria[0].min_values[:time], c="r", label="Error ratio's minimum value")
    color = ["r", "g", "b"]
    for c, criterion in enumerate(al.stopping_criteria):
        if time >= criterion.stop_timings:
