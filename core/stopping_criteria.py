...
class error_stability_criterion(base_criterion):
    def __init__(self, threshold, validate_size=1, normalize_type="min"):
    ...
    def check_threshold(self, KL_pq, KL_qp, current_time):
        ...
        if self.validate_size <= current_time:
            if self.normalize_type == "min":
                error_ratio = self.R[-1] / self.R_min[self.validate_size - 1]
            elif self.normalize_type == "actual":
                error_ratio = self.R[-1] / self.R[:self.validate_size + 1].min()
            else:
                error_ratio = self.R[-1]
            if self.validate_size == current_time:
                error_ratio = 1
    ...
    def calcR_min(self, KL_pq_min, KL_qp_min, current_time):
        ...
        if self.validate_size <= current_time:
            if self.normalize_type == "min":
                min_values = self.R_min[-1] / self.R_min[self.validate_size - 1]
            elif self.normalize_type == "actual":
                min_values = self.R_min[-1] / self.R[:self.validate_size + 1].min()
            else:
                min_values = self.R_min[-1]
            if self.validate_size == current_time:
                min_values = 1
