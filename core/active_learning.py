...
class AL(object):
    def __init__(self, X_pool, y_pool, init_sample_size, stopping_criteria, kernel, normalize_y=False,
                 optimizer="fmin_l_bfgs_b",n_restarts_optimizer=0, copy_X_train=False, max_iter=2e05, gtol=1e-06):
        self.gp_params={"kernel":kernel,"alpha":0,"optimizer":optimizer,"n_restarts_optimizer":n_restarts_optimizer,
    ...
    def data_aquire(self,gp,pool_data,pool_indecies,current_time):
    ...
    def explore(self,max_iters=100):
        ...
        for t in tqdm(range(1, self.sample_size + 1)):
            # acquire new input
            new_data_index = self.data_aquire(gp,self.X_pool, self.pool_indecies, t)
